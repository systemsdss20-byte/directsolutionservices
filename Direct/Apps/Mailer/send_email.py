            ...
            if type(self.recipients) != list:
                ...
                for recipient in self.recipients:
                    if email_sent == 28:
                        time.sleep(120)
                        email_sent = 0
                    subject = self.subjects.pop() if type(self.subjects) == list else self.subjects
                    if validate_email(recipient):
                        sent = self.__send(
                            recipient=recipient,
                            subject=subject,
                            context=self.context.pop() if type(self.context) == list else self.context
                        )
    ...
    def __send(self, recipient, subject, context):
        try:
            ...
            email = EmailMultiAlternatives(
                subject=subject,
                body=plain_message,
                from_email='Direct Solution Services<dot@compliance-dss.com>',
